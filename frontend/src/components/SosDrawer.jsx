...
export default function SosDrawer({ isOpen, onClose, trustedContact }) {
          ...
          {!alertSent ? (
            <div className="text-center space-y-5 py-4">
              <p className="text-xs text-secondary-text leading-relaxed">
                Tapping the emergency button below will immediately broadcast
                your live GPS coordinates to your family contact and alert the
                RideLink safety response team.
              </p>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex gap-3 text-red-800 text-xs leading-relaxed">
                ...
                <div>
                  ...
                  <p className="font-semibold text-red-700">
                    We successfully transmitted your live coordinates{" "}
                    <strong className="text-red-900">
                      9.9816° N, 76.2999° E
                    </strong>{" "}
                    to your trusted contact{" "}
                    {trustedContact
                      ? `(${trustedContact.relationship}: ${trustedContact.name})`
                      : "family contacts"}{" "}
                    and dispatched an alert to our safety monitors.
                  </p>
                </div>
              </div>
              ...
              {smsSimulated && (
                ...
                >
                  <span className="absolute top-2 right-3 text-[8px] bg-red-600 text-white px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
                    SMS Broadcast Sent
                  </span>
                  <p className="font-bold text-slate-400 mb-1">
                    Preview of SMS sent to {trustedContact?.name || "Contact"}:
                  </p>
                  <p className="text-slate-200">
                    "ALERT: Sarah Prasad has triggered emergency SOS on her
                    RideLink trip to Kannur. Track coordinates live:
                    http://localhost:3000/?track=ride_1&alert=sos"
                  </p>
                </motion.div>
              )}
              ...
              <div className="bg-slate-50 border p-4 rounded-2xl space-y-2.5 text-xs text-secondary-text">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-slate-800 flex items-center gap-1">
                    <MapPin size={13} className="text-red-500" /> 9.9816° N,
                    76.2999° E
                  </span>
                </div>
                {trustedContact && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-slate-800">
                      {trustedContact.name} ({trustedContact.relationship})
                    </span>
                  </div>
                )}
                <div className="flex items-center justify-between">
