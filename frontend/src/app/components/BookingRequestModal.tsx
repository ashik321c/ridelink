...
import { Star, CheckCircle, MapPin, MessageSquare, Phone, X, ShieldCheck } from 'lucide-react';
...
interface BookingRequestModalProps {
  ride: Ride;
  onClose: () => void;
  onSubmitBooking: (seats: number, pickup: string, message: string) => Promise<{ success: boolean; booking?: { id: string } | null } | null>;
  onNavigateToDashboard: () => void;
}

type ModalState = 'review' | 'sending' | 'confirmed';
...
export default function BookingRequestModal({ 
  ...
  const [passengerMessage, setPassengerMessage] = useState("Hi, I'd like to join your ride.");
  const [loading, setLoading] = useState(false);
  const [bookingId, setBookingId] = useState<string | null>(null);

  // Derive mock registration number and completed rides
  const regNumber = ride.driverId === 'user_1' ? "KL-07-CD-1234" : "KL-11-AA-5678";
      ...
      const timer = setTimeout(async () => {
        try {
          if (bookingId) {
            ...
            }).catch(() => console.warn("Mock status update failed, continuing demo."));
          }
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await onSubmitBooking(seatsRequested, pickupPoint, passengerMessage);
              ...
              <div className="bg-gradient-to-r from-blue-50/50 to-indigo-50/30 p-6 flex items-center gap-4 border-b border-brand-border pt-7">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <h3 className="text-base font-extrabold text-primary-text">{ride.driverName}</h3>
                    {/* Blue Verified Badge */}
                    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-blue-50 text-[9px] font-bold text-primary-blue rounded-md border border-blue-100">
                      <ShieldCheck size={11} className="fill-blue-500 text-white" /> Verified Driver
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-secondary-text">
                    <span className="flex items-center gap-0.5">
                      ...
                      <strong className="text-slate-800 font-bold">{ride.driverRating}</strong> Rating
                    </span>
                    <span>•</span>
                    <span className="font-semibold">{completedRides} Completed Rides</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5">
                <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl space-y-3.5">
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <span className="text-secondary-text block text-[10px] font-bold uppercase tracking-wider mb-0.5">From</span>
                      <span className="font-bold text-primary-text text-sm">{ride.origin}</span>
                    </div>
                    <div>
                      <span className="text-secondary-text block text-[10px] font-bold uppercase tracking-wider mb-0.5">To</span>
                      <span className="font-bold text-primary-text text-sm">{ride.destination}</span>
                    </div>
                  </div>

                  <div className="border-t border-slate-200/60 pt-3 grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <span className="text-secondary-text block text-[10px] font-bold uppercase tracking-wider mb-0.5">Departure Details</span>
                      <span className="font-semibold text-primary-text">{ride.date} at {ride.time} AM</span>
                    </div>
                    <div>
                      <span className="text-secondary-text block text-[10px] font-bold uppercase tracking-wider mb-0.5">Vehicle Details</span>
                      <span className="font-semibold text-primary-text block truncate">{ride.vehicle}</span>
                      <span className="text-[10px] text-slate-400 font-bold tracking-wider">{regNumber}</span>
                    </div>
                  </div>

                  <div className="border-t border-slate-200/60 pt-3 text-xs flex justify-between">
                    <div>
                      <span className="text-secondary-text text-[10px] font-bold uppercase tracking-wider block mb-0.5">Available Seats</span>
                      <span className="font-semibold text-primary-text">{ride.seatsAvailable} seats left</span>
                    </div>
                    <div className="text-right">
                      <span className="text-secondary-text text-[10px] font-bold uppercase tracking-wider block mb-0.5">Contribution Per Seat</span>
                      <span className="font-extrabold text-primary-blue text-sm">₹{ride.contributionAmount}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between border-t border-brand-border pt-5 mt-2">
                  <div>
                    <span className="text-[10px] uppercase font-bold text-secondary-text tracking-wider">Total Ride Contribution</span>
                    <p className="text-2xl font-extrabold text-primary-blue">₹{ride.contributionAmount * seatsRequested}</p>
                  </div>
            ...
            <div className="p-8 text-center space-y-8">
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <h3 className="text-xl font-extrabold text-primary-text">Your request has been sent.</h3>
                  <p className="text-xs text-secondary-text font-medium">Waiting for driver confirmation...</p>
                </div>
              </div>

              <div className="bg-slate-50 border border-slate-100 p-5 rounded-2xl text-left space-y-4">
                <h4 className="text-[10px] uppercase tracking-wider font-extrabold text-secondary-text">Booking Process</h4>
                <div className="relative flex flex-col space-y-5">
                  <div className="absolute left-[9px] top-2 bottom-2 w-0.5 bg-slate-200" />
                  
                  <div className="flex items-center gap-3 relative">
                    <div className="w-5 h-5 rounded-full bg-brand-success text-white flex items-center justify-center text-[10px] font-bold z-10">✓</div>
                    <span className="text-xs font-semibold text-secondary-text">Search Ride</span>
                  </div>

                  <div className="flex items-center gap-3 relative">
                    <div className="w-5 h-5 rounded-full bg-primary-blue text-white flex items-center justify-center text-[10px] font-bold z-10 animate-pulse">✓</div>
                    <span className="text-xs font-bold text-primary-blue">Request Sent</span>
                  </div>

                  <div className="flex items-center gap-3 relative">
                    <div className="w-5 h-5 rounded-full bg-slate-200 text-slate-400 flex items-center justify-center text-[10px] font-bold z-10">3</div>
                    <span className="text-xs font-medium text-slate-400">Driver Accepted</span>
                  </div>
                </div>
              </div>
          ...
          {modalState === 'confirmed' && (
            <div className="p-8 text-center space-y-8">
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <h3 className="text-2xl font-extrabold text-brand-success">Ride Confirmed ✅</h3>
                  <p className="text-xs text-secondary-text font-medium leading-relaxed max-w-sm mx-auto">
                    Great news! {ride.driverName} has accepted your ride request. You are ready to travel.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-100 p-5 rounded-2xl text-left space-y-3.5">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="text-secondary-text">Pickup Location:</span>
                    <span className="font-bold text-primary-text flex items-center gap-1"><MapPin size={13} className="text-primary-blue" /> {pickupPoint}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-secondary-text">Driver Contact:</span>
                    <span className="font-bold text-primary-text flex items-center gap-1"><Phone size={13} className="text-slate-400" /> +91 98765 43210</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-secondary-text">Vehicle Reg Number:</span>
                    <span className="font-bold text-slate-700 bg-slate-200 px-1.5 py-0.5 rounded font-mono text-[10px]">{regNumber}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-secondary-text">Total Contribution:</span>
                    <span className="font-extrabold text-brand-success">₹{ride.contributionAmount * seatsRequested}</span>
                  </div>
                </div>
              </div>
